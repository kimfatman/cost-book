...
`{ id, name, color }` 数组。7 个预设：食材采购（#1677FF）、人力工资（#12B76A）、房租水电（#F79009）、营销推广（#7F56D9）、物流仓储（#F04438）、设备折旧（#0B1836）、其他（#A3AEC2）。`color` 用于分类色点与图表着色（`catColor(name)` 按名称查色）。行业模板分类色板统一为新品牌色（8 色板见 docs/10 §1.3）。
