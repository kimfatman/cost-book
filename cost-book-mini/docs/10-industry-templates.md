...
### 1.3 分类配色板（✅ 已统一，v2.4.5）

- 8 个行业模板 + 默认 `DB.categories` 全部使用新品牌色板：`#1677FF` 蓝 / `#12B76A` 绿 / `#F79009` 橙 / `#7F56D9` 紫 / `#F04438` 红 / `#0B1836` 深蓝 / `#667085` 灰 / `#A3AEC2` 浅灰
- 切换行业不再出现分类色跳变（旧色板已全部移除）。
...
| # | 现象 | 原因 | 建议 |
|---|---|---|---|
| 1 | 切换行业后旧分类的记账记录保留 | **已修复（v2.4.5）**：`apply()` 自动把仍被记录引用的旧分类追加为自定义分类，不再灰色 fallback | — |
...
| 4 | ~~分类配色新旧两套并存~~ | **已修复（v2.4.5）**：8 模板 + 默认分类全部统一新色板 | — |
| 5 | ~~隐藏行业（fresh/factory/service）无法经 UI 选择~~ | **已修复（v2.4.5）**：`buildStep1` keys 已开放全部 8 个行业 | — |
